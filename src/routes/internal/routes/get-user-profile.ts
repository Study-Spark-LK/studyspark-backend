...
export function setupGetUserProfileRoute() {
	...
	app.get('/internal/users/:userId', async (c) => {
		...
		return c.json({
			...
			visual_score: profile.visualScore ?? 0,
			auditory_score: profile.auditoryScore ?? 0,
			reading_score: profile.readingScore ?? 0,
			kinesthetic_score: profile.kinestheticScore ?? 0,
